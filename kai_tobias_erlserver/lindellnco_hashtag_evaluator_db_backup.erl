...
-export([sample_to_db/4, evaluate_hashtag/1,run/0, get_last_tweets/2,get_hashtag_tweets/2, hashtag_url/2,append_binary/1
        ]).
...
%   TWEETLIST: A list of tweets, in the format as given by twitter
get_hashtags_from_tweets(TweetList) -> [{extract_hashtags(T),get_field(<<"id">>,T)}|| T <- TweetList].
...
evaluate_hashtag(BitString) -> 
  ...
  MultipleTweetList= repeat_get_hashtag_tweets(BitString,1,MaxId),
...
sample_to_db(PairDb, IdDb, Xsearch, Ysearch) -> 
  io:format("~n~n~n~p~n~p~n~n~n",[Xsearch, Ysearch]),
  Rx = evaluate_hashtag(Xsearch),
  io:format("Rx Length:~p~n",[length(Rx)]),
  %io:format("Rx:~p~n",[Rx]),
  add_all_to_db(PairDb,IdDb, Rx),
  io:format("X Done ~n"),
  Ry = evaluate_hashtag(Ysearch),
  io:format("Ry Length:~p~n",[length(Ry)]),
  add_all_to_db(PairDb,IdDb, Ry),
  io:format("Y Done ~n").
 % io:format("Ry:~p~n",[Ry]).

add_all_to_db(_,_, []) -> ok;
add_all_to_db(PairDb, IdDb, [{Hash,Id}|T]) ->
  Pairs = [{X, Y} || X <- Hash, Y <- Hash],
  case Pairs of
    [] ->
      ok;
    _ -> 
      %io:format("Pairs:~p~n",[Pairs]),
      %%%%% CHECK TWEET NOT ALREADY THERE!!!
      Doc_Id = {[
        {<<"_id">>, list_to_binary(integer_to_list(Id))}
      ]},
      connection:make_doc(IdDb,Doc_Id),
      connection:write_hashtags_to_db(PairDb,Pairs),
      io:format("~p",[length(Pairs)]),
      add_all_to_db(PairDb, IdDb,T)
  end.
